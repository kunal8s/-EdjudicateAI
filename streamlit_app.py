...
if not backend_available:
    ...
    <div class="warning-message">
        ⚠️ <strong>Demo Mode:</strong> Backend server is not available. This is a demonstration of the UI interface. 
        For full functionality, please deploy the backend server or run locally.
    </div>
    """, unsafe_allow_html=True)
...
if uploaded_files:
    ...
    if backend_available:
        ...
        with st.spinner("🔄 Processing and indexing documents..."):
            ...
            try:
                response = requests.post(f"{API_URL}/upload_docs", files=files_data, timeout=30)
        ...
        st.markdown(f"""
        <div class="success-message">
            ✅ Demo mode: Files uploaded successfully! (Session ID: {session_id})
        </div>
        """, unsafe_allow_html=True)
    ...
    if not session_id:
        ...
        if backend_available:
            ...
            demo_response = {
                "decision": "approved",
                "amount": 50000,
                "justification": "This is a demo response. In a real deployment, the AI would analyze the uploaded documents and provide a detailed analysis based on the policy clauses."
            }
            ...
            st.markdown("""
            <div class="section-header" style="margin-top: 2rem;">
                📋 Referenced Clauses (Demo)
            </div>
            """, unsafe_allow_html=True)
            
            demo_clauses = [
                "This is a sample clause demonstrating how the AI would reference specific policy sections.",
                "In a real deployment, the AI would extract and display relevant clauses from the uploaded documents."
            ]
